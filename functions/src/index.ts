...
interface NotifyGameConfirmedRequest {
  groupId: string;
  gameDate: string;
  gameType: string; // 'singles' | 'doubles'
  players: string[]; // Player names in the game
  excludePlayer?: string; // Don't notify this player (the one who triggered confirmation)
}
 ...
 */
export const notifyGameConfirmed = functions.https.onCall(
  async (data: NotifyGameConfirmedRequest, context) => {
    const { groupId, gameDate, gameType, players, excludePlayer } = data;

    if (!groupId || !gameDate || !gameType || !players) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Missing required fields: groupId, gameDate, gameType, players'
      );
    }
    ...
    try {
      ...
      for (const playerName of players) {
        ...
        for (const [memberId, prefs] of Object.entries(allPrefs)) {
          const memberPrefs = prefs as {
            pushEnabled?: boolean;
            pushToken?: string;
            gameConfirmed?: string;
          };

          // Check if this member matches and has push enabled
          if (
            memberId.toLowerCase().includes(normalizedName) ||
      ...
      console.log(
        `[notifyGameConfirmed] Sent: ${response.successCount} success, ${response.failureCount} failed`
      );
