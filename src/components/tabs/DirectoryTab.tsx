...
function getLastPlayDate(memberName: string): Date | null {
  ...
  Object.keys(checkins).forEach(dateStr => {
    ...
    if (checkDate <= today) {
      ...
      const memberCheckedIn = dateCheckins.some((checkin: any) =>
        checkin && checkin.name === memberName
      );
    ...
    <div style="padding: var(--spacing-2xl, 16px) 0;">
          ...
          if (filteredMembers.length === 0) {
            return (
              <div style={{
                ...
                <svg viewBox="0 0 24 24" width="48" height="48" fill="var(--color-border-light, #ddd)" style={{ marginBottom: 'var(--spacing-xl, 12px)' }}>
          ...
          return filteredMembers.map(memberName => {
            ...
            return (
              <div
                key={memberName}
                style={{
                  background: isCurrentUser ? 'var(--color-primary-light, #e8f5e9)' : 'var(--color-bg-card, #fff)',
                  borderRadius: 'var(--radius-xl, 12px)',
                  padding: 'var(--spacing-2xl, 16px)',
                  border: isCurrentUser ? '2px solid var(--color-primary, #2C6E49)' : '1px solid var(--color-border, #e0e0e0)',
                }}
              >
                <div style={{ display: 'flex', alignItems: 'start', justifyContent: 'space-between', gap: 'var(--spacing-xl, 12px)' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {/* Name */}
                    <div style={{ fontWeight: 600, fontSize: 'var(--font-size-lg, 16px)', color: 'var(--color-text-primary, #333)', marginBottom: 'var(--spacing-md, 8px)' }}>
                      {memberName}
                      {isCurrentUser && <span style={{ marginLeft: 'var(--spacing-md, 8px)', fontSize: 'var(--font-size-sm, 12px)', color: 'var(--color-primary, #2C6E49)', fontWeight: '500' }}>(You)</span>}
                    </div>
                    ...
                    {lastPlayDate && (
                      <div style={{
                        ...
                        <svg viewBox="0 0 24 24" width="14" height="14" fill="var(--color-text-muted, #888)">
                    ...
                    {!lastPlayDate && (
                      <div style={{ fontSize: 'var(--font-size-sm, 13px)', color: 'var(--color-text-muted, #999)', marginBottom: 'var(--spacing-md, 8px)' }}>
                        No recent games
                      </div>
                    )}
                    ...
                    {details?.addedBy && (
                      <div style={{ fontSize: 'var(--font-size-sm, 12px)', color: 'var(--color-text-muted, #999)', marginBottom: 'var(--spacing-md, 8px)' }}>
                        Added by {details.addedBy}
                        {details.addedDate && ` • ${new Date(details.addedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`}
                      </div>
                    )}

                    {/* Contact Buttons (only if member opted in) */}
                    {hasContact && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md, 8px)', marginTop: 'var(--spacing-md, 8px)', flexWrap: 'wrap' }}>
                        {details.phone && (
                          <>
                            {/* WhatsApp */}
                            <a
                  ...
                  {(isCurrentUser || adminStatus) && (
                    <button
                      ...
                      style={{
                        background: isCurrentUser ? 'var(--color-primary, #2C6E49)' : 'var(--color-bg-muted, #f5f5f5)',
                    ...
                    >
                      <svg viewBox="0 0 24 24" width="16" height="16" fill={isCurrentUser ? 'white' : 'var(--color-text-secondary, #666)'} style={{ marginRight: 'var(--spacing-xs, 4px)' }}>
      ...
      {coreMembers.value.length > 0 && !searchQuery && (
        <div style={{
          ...
          <div style={{ fontSize: 'var(--font-size-sm, 13px)', color: 'var(--color-text-secondary, #666)' }}>
            <span style={{ fontWeight: '600', color: 'var(--color-text-primary, #333)' }}>{coreMembers.value.length}</span>
